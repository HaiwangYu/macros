...
void Svtx_Reco(int verbosity = 0)
{
  ...
  PHG4TrackKalmanFitter *kalman = new PHG4TrackKalmanFitter();
	//kalman->set_output_mode(PHG4TrackKalmanFitter::DebugMode);//MakeNewNode, OverwriteOriginalNode, DebugMode, defualt is OverwriteOriginalNode
	//kalman->set_mag_field_file_name("/phenix/upgrades/decadal/fieldmaps/sPHENIX.2d.root");
	kalman->set_fit_primary_tracks(true); // default is true
	kalman->set_do_eval(true); // default is false
	kalman->set_vertexing_method("kalman-smoothing:1");
	//kalman->set_vertexing_method("mvf-smoothing:1");
	//kalman->set_vertexing_method("mvf");
	//kalman->Verbosity(10);
